...
namespace LinqExercicesSolutions
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            #region EX 4.9 (Cross Join)
            {
                ...
                //var Result49Bis = dc.Students.Where(st => st.Year_Result >= 12)
                //                             .Join(dc.Grades, st => 1, gr => 1, (st, grade) => new { Student = st, Grade = grade })
                //                             .Where(join => join.Student.Year_Result >= join.Grade.Lower_Bound && join.Student.Year_Result <= join.Grade.Upper_Bound)
            ...
            #region EX 4.10 (Très Très HARD!!!)
            {
                ...
                //                                    Section_Name = (pcs.Section is not null) ? pcs.Section.Section_Name : null,
                //                                    Course_Name = (pcs.Course is not null) ? pcs.Course.Course_Name : null,
                //                                    Course_Ects = (pcs.Course is not null) ? (float?)pcs.Course.Course_Ects : null
                //                                })
